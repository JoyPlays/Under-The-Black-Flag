	...
	SubShader 
	{
		...
		////// Shader functions
			...
			{				
				...
				// Depth fog
				float2 depthValues = GetDepthValues(fragIn.posScreen, _CameraDepthTexture, _ShallowDeepFade, _ShallowDepth, _EdgeFade);
				...
				// Foam
				float4 foam = GetFoam(_FoamTex, fragIn.uv_Environment.zw, groundLevel, _FoamDistance);
				...
				final.w = max(max(max(depthLevel, specularLight.x), foam.a), _ShoreEdgeIndicator);
