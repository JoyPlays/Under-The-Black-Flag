	...
	SubShader 
	{
		...
		////// Shader functions
			...
			float4 frag(Vert2Frag fragIn) : SV_Target
			{
				...
				float4 reflection = GetReflection(fragIn.posScreen, _ReflectionTex, float3(0,1,0), _Refraction) * reflectivity;
				
				// Final composition
				float4 final = float4( saturate(tex2D(_MainTex, fragIn.uv_Tex.xy).xyz*(1-reflectivity) + reflection), _Transparency) ;
