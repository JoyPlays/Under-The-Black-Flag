	...
	private void UpdateRender()
	{
		...
		foreach (Material mat in materials)
		{
			if (mat.HasProperty("_UVOffset"))
			{
				...
				offset += (Mathf.Sin(Time.time) * 0.2f +Time.time) * new Vector4(movement.x, movement.y, 0, 0);
				offset += (Mathf.Cos(Time.time) * 0.2f + Time.time) * new Vector4(0, 0, movement.z, movement.w);
		...
		Vector3 normal = GetComponent<MeshFilter>() != null ? transform.TransformDirection(GetComponent<MeshFilter>().sharedMesh.normals[0]) : transform.up;
